...
<body>
	...
	<%--
		...
		request : 현재 페이지 또는 다음 페이지까지 유지되는 값
			단, request는 다음 페이지까지 전달하고자 하면 forword 사용해야함
		session : 웹 브라우저가 살아있는 동안 유지된다.
		application : 서버가 살아있는 동안 유지 된다.
	--%>
	<%
		pageContext.setAttribute("name", "page value");
		request.setAttribute("name", "request value");
		session.setAttribute("name", "session value");
		application.setAttribute("name", "applcation value");
	%>
	
	name : ${name }<br/>
	page : <%=pageContext.getAttribute("name") %>, ${pageScope.name }<br/>
